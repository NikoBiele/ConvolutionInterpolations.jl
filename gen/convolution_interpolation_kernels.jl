...
for (k, c) in zip(a_kernels, a_colors)
    itp = convolution_interpolation(x_data, y_data; kernel=k)
    plot!(p1, x_fine, itp.(x_fine), label="kernel=:$k", color=c, linewidth=1.2)
end
plot!(p1, title="Lower order kernels", xlabel="x", ylabel="y")

# --- Right panel: ':b' type kernels ---
...
for (k, c) in zip(b_kernels, b_colors)
    itp = convolution_interpolation(x_data, y_data; kernel=k)
    plot!(p2, x_fine, itp.(x_fine), label="kernel=:$k", color=c, linewidth=1.2)
end
