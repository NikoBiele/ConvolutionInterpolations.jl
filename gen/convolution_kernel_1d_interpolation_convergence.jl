...
itpb3_fast_errors = zeros(G, length(ns))
...
for n in ns
    ...
    itpb3_fast = convolution_interpolation(xr, runge.(xr); degree=:a4, fast=true, subgrid=:cubic, kernel_bc=[(:polynomial,:polynomial)]);
    ...
    itpb3_fast_errors[count] = maximum(abs.(itpb3_fast.(xf) - yf))
...
# Add legend
Legend(fig[1, 2], ax, fontsize=12)

fig

# save("interpolation_1d_runge.png", fig, px_per_unit=3)  # High DPI
