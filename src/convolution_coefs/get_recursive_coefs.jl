...
"""
    get_recursive_coefs(y_centered::NTuple{NS,T}, h::T, bc::Symbol, which_end::Symbol) where {NS,T}
...
- `h::T`: Grid spacing
- `bc::Symbol`: Boundary condition type (`:detect`, `:linear`, `:quadratic`, `:periodic`)
...
# Methods
- `:detect`: prioritizes `:poly`, falls back to `:linear`
...
function get_recursive_coefs(y_centered::NTuple{NS,T}, h::T, bc::Symbol, which_end::Symbol) where {NS,T}
    if bc == :linear || bc == :detect || bc == :poly # fallback
        return (T(2), T(-1), T(0), T(0), T(0))
    elseif bc == :periodic
        return periodic_boundary(y_centered)
    elseif bc == :quadratic
        return (T(3), T(-3), T(1), T(0), T(0))
    else
        error("Unsupported kernel boundary condition: kernel_bc = $bc. " *
              "Supported: :poly, :detect, :linear, :quadratic, :periodic or :predict.")
    end
end