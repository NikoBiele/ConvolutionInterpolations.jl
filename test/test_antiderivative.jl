...
@testset "1D antiderivative convergence fast" begin
    for kernel in antideriv_kernels_all
        ...
        for n in [24, 48, 96]
            ...
            vals = sin.(collect(r))
            itp = convolution_interpolation(r, vals; degree=kernel, derivative=-1,
                                            fast=true, kernel_bc=:polynomial)
...
@testset "1D antiderivative convergence direct" begin
    for kernel in antideriv_kernels_all
        ...
        for n in [24, 48, 96]
            ...
            vals = sin.(collect(r))
            itp = convolution_interpolation(r, vals; degree=kernel, derivative=-1,
                                            fast=false, kernel_bc=:polynomial)
            anchor = itp.itp.anchor[1]
            exact(x) = -cos(x) + cos(anchor)
            test_pts = interior_frac_1d(collect(r))
            err = maximum(abs(itp(x) - exact(x)) for x in test_pts)
...
@testset "1D antiderivative fast vs direct agreement" begin
    for kernel in antideriv_kernels_all
        ...
        r = range(0.0, 2π, length=n)
        vals = sin.(collect(r))
        itp_fast   = convolution_interpolation(r, vals; degree=kernel, derivative=-1,
                                               fast=true,  kernel_bc=:polynomial)
        itp_direct = convolution_interpolation(r, vals; degree=kernel, derivative=-1,
                                               fast=false, kernel_bc=:polynomial)
        test_pts = interior_frac_1d(collect(r))
...
@testset "1D antiderivative anchor is zero" begin
    for kernel in antideriv_kernels_all
        ...
        for fast in [true, false]
            itp = convolution_interpolation(r, vals; degree=kernel, derivative=-1,
                                            fast=fast, kernel_bc=:polynomial)
            anchor = itp.itp.anchor[1]
...
@testset "2D antiderivative convergence fast" begin
    for kernel in [:a3, :b5, :b7, :b9]
        ...
        for n in [20, 40, 80]
            ...
            itp = convolution_interpolation((xs, ys), zs; degree=kernel, derivative=-1,
                                            fast=true, kernel_bc=:polynomial)
...
@testset "3D antiderivative convergence fast" begin
    for kernel in [:a3, :b5, :b7]
        ...
        for n in [12, 24, 48]
            ...
            itp = convolution_interpolation((xs, ys, zs), vs; degree=kernel, derivative=-1,
                                            fast=true, kernel_bc=:polynomial)
