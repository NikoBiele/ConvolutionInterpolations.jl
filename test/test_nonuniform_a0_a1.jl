...
@testset "Nonuniform a0 nearest-neighbor" begin
    ...
    vs = sin.(knots)
    itp = convolution_interpolation(knots, vs, degree=:a0)
    ...
    itp_2d = convolution_interpolation((kx, ky), vs_2d, degree=:a0)
    for (i, x) in enumerate(kx), (j, y) in enumerate(ky)
        @test itp_2d(x, y) ≈ vs_2d[i, j] atol=1e-4
    end
end
...
@testset "Nonuniform a1 linear interpolation" begin
    knots = [0.0, 0.3, 0.7, 1.2, 1.5, 2.0]
    vs = sin.(knots)
    itp = convolution_interpolation(knots, vs, degree=:a1)
    ...
    for i in 1:length(knots)-1
        ...
        @test itp(xm) ≈ (vs[i] + vs[i+1]) / 2 atol=1e-4
    end

    # 1D linear data: exact everywhere
    vs_lin = Float64.(1:6)
    itp_lin = convolution_interpolation(knots, vs_lin, degree=:a1)
    for x in [0.1, 0.5, 0.85, 1.35, 1.8]
        ...
        @test itp_lin(x) ≈ expected atol=1e-4
    end
...
@testset "Nonuniform a0/a1 lazy fields" begin
    ...
    itp_a0 = convolution_interpolation(knots, vs, degree=:a0)
    @test itp_a0.itp.lazy == true
    @test itp_a0.itp.coefs === vs

    itp_a1 = convolution_interpolation(knots, vs, degree=:a1)
